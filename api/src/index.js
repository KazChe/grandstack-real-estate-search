...
/*
   KC - adding a resolver for getting cities by UUIDs

query {
  citiesByUUID(uuid:["a40a5d72-c9c0-49c6-af79-29b68a60d844",
    "a01ce3b3-3da0-4e4c-bc1c-0fb64695ca4c"])
  {
    name
    uuid
    _id
...
const reslovers = {
  CitiesByUUID: {
    cities: async (ids, args) => {
      ...
      const uuIds = split(ids)
      const result = await session.run(
          `CALL apoc.cypher.run('MATCH(c:City) WHERE c.uuid IN [${ids}] RETURN c', {}) YIELD value RETURN value`
      )
      return result
    }
  }
